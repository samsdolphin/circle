...
void PathPlanner::path_gen(const TimeSeries& knots,
                           const nav_msgs::Path& waypoints,
                           const geometry_msgs::Twist& v0,
                           const geometry_msgs::Twist& a0,
                           TrajGenOpts opt)
{    
    ...
    cout<<"[PathPlanner] segments completed"<<endl;
...
QP_form_xyz PathPlanner::qp_gen(const TimeSeries& knots,
                                const nav_msgs::Path& waypoints,
                                const geometry_msgs::Twist& v0,
                                const geometry_msgs::Twist& a0,
                                TrajGenOpts opt)
...
void PathPlanner::horizon_eval_spline(int N_eval_interval)
{
    ...
    cout<<"[PathPlanner] evaluating spline, num_segment: "<<n_seg<<", poly_order: "<<poly_order<<endl;
...
VectorXd PathPlanner::solveqp(QP_form qp_prob, bool& is_ok)
{
    ...
    {
        cerr<<"[QP solver] problem is infeasible"<<endl;
        is_ok = false;
    }
    real_t xOpt[N_var];
    ...
    {
        cerr<<"[QP solver] quadratic programming has not been solved"<<endl;
        is_ok = false;
    }
